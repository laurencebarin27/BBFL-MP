...
<div class="w3layouts-main">
	...
	if (isset($_POST['login'])){
		...
		if (empty($message)){
			 ...
			 $resultlogin=mysqli_query($dbc,$loginquery);
			 $num_rows=$resultlogin->num_rows;

			 if (!empty($num_rows)){

			 	while ($loginrow = mysqli_fetch_array($resultlogin,MYSQL_ASSOC)){
			 		if ("{$loginrow['username']}" == $_SESSION['user'] && "{$loginrow['password']}" == $_SESSION['pass']){
